...
if config.features.screenshot_tools then
  -- Current monitor to clipboard { Print }
  ...
  -- Current monitor to file { Shift + Print }
  table.insert(global_bindings, binding.new {
    modifiers = { mod.super },
    triggers = "Print",
    path = { "Screenshot", "To File" },
    description = "Current monitor",
    on_press = function() services.screenshot.take { mode = "screen", output = "file" } end,
  })
  -- Active window to clipboard { Ctrl + Print }
  table.insert(global_bindings, binding.new {
  ...
  -- Active window to file { Shift + Ctrl + Print }
  table.insert(global_bindings, binding.new {
    modifiers = { mod.control, mod.super },
    triggers = "Print",
    path = { "Screenshot", "To File" },
    description = "Active window",
    on_press = function() services.screenshot.take { mode = "window", output = "file" } end,
  })
  -- Rectangular selection to clipboard { Ctrl + Super + Print }
  table.insert(global_bindings, binding.new {
  ...
  -- Rectangular selection to file { Shift + Ctrl + Super + Print }
  table.insert(global_bindings, binding.new {
    modifiers = { mod.control, mod.super, mod.shift },
    triggers = "Print",
    path = { "Screenshot", "To File" },
    description = "Rectangular selection",
    on_press = function() services.screenshot.take { mode = "selection", output = "file" } end,
  })
end
